...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None, filed_cluster_points=None, filed_cluster_points_local=None, filtered_mask = None):
    """
    Render the scene. 
    ...
    colors_precomp = None
    rendered_image = None
    rendered_image_filed = None
    rendered_image_filed_local = None
    ...
    if filed_cluster_points is None:
        ...
        rendered_image_filed, radii = rasterizer(
            means3D = means3D[filed_cluster_points],
            means2D = means2D[filed_cluster_points],
            shs = shs[filed_cluster_points],
            colors_precomp = colors_precomp,
            opacities = opacity[filed_cluster_points],
            scales = scales[filed_cluster_points],
            rotations = rotations[filed_cluster_points],
            cov3D_precomp = cov3D_precomp)
        
        if filed_cluster_points_local is not None:
            rendered_image_filed_local, radii = rasterizer(
                means3D = (means3D[filed_cluster_points])[filed_cluster_points_local],
                means2D = (means2D[filed_cluster_points])[filed_cluster_points_local],
                shs = (shs[filed_cluster_points])[filed_cluster_points_local],
                colors_precomp = colors_precomp,
                opacities = (opacity[filed_cluster_points])[filed_cluster_points_local],
                scales = (scales[filed_cluster_points])[filed_cluster_points_local],
                rotations = (rotations[filed_cluster_points])[filed_cluster_points_local],
                cov3D_precomp = cov3D_precomp)

    return {"render": rendered_image,
            "render_filed": rendered_image_filed,
            "render_filed_local": rendered_image_filed_local,
...
def render_contrastive_feature(viewpoint_camera, pc : FeatureGaussianModel, pipe, bg_color : torch.Tensor, filed_cluster_points=None, scaling_modifier = 1.0, norm_point_features = False, smooth_type = None, smooth_weights = None, smooth_K = 16):
    """
    Render the scene. 
    ...
    if filed_cluster_points is None:
        ...
        rendered_image, radii = rasterizer(
            ...
            means2D = means2D[filed_cluster_points],
            shs = shs,
            colors_precomp = colors_precomp[filed_cluster_points],
            opacities = opacity[filed_cluster_points],
            scales = scales[filed_cluster_points],
            rotations = rotations[filed_cluster_points],
            cov3D_precomp = cov3D_precomp)
