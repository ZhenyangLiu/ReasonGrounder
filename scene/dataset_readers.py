...
def readCamerasFromLerfTransforms(path, transformsfile, white_background, extension=".jpg", masks_folder = None, mask_scale_folder = None):
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in enumerate(frames):
            ...
            masks = torch.load(os.path.join(masks_folder, frame["file_path"].split(".")[1].split("/")[2] + ".pt")) if masks_folder is not None else None
            mask_scales = torch.load(os.path.join(mask_scale_folder, frame["file_path"].split(".")[1].split("/")[2] + ".pt")) if mask_scale_folder is not None else None
            cam_infos.append(CameraInfo(uid=idx, R=R, T=T, FovY=FovY, FovX=FovX, image=image,
...
def readLerfInfo(path, white_background, eval, extension=".png"):
    ...
    train_cam_infos = readCamerasFromLerfTransforms(path, "transforms.json", white_background, extension, masks_folder=os.path.join(path, mask_dir), mask_scale_folder=os.path.join(path, mask_scale_dir))
    # print("Reading Test Transforms")
    # test_cam_infos = readCamerasFromTransforms(path, "transforms_test.json", white_background, extension)
    test_cam_infos = []
    eval = False
