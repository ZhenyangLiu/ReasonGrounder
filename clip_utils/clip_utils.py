...
class OpenCLIPNetworkConfig:
    ...
    clip_model_pretrained: str = "./clip_ckpt/open_clip_pytorch_model.bin"
    clip_n_dims: int = 512
...
class OpenCLIPNetwork(nn.Module):
    ...
    def set_scale_positives(self, text_list):
        self.positives = text_list
        with torch.no_grad():
            tok_phrases = torch.cat([self.tokenizer(phrase) for phrase in self.positives]).to("cuda")
            self.pos_embeds = self.model.encode_text(tok_phrases)
    ...
    # def get_scale_relevancy(self, embed: torch.Tensor, positive_id: int) -> torch.Tensor:
    #     phrases_embeds = torch.cat([self.pos_embeds, self.neg_embeds], dim=0)
    #     p = phrases_embeds.to(embed.dtype)  # phrases x 512
    #     output = torch.mm(embed.cuda(), p.T)  # rays x phrases
    #     positive_vals = output[..., positive_id : positive_id + 1]  # rays x 1
    #     negative_vals = output[..., len(self.pos_embeds) :]  # rays x N_phrase
    #     repeated_pos = positive_vals.repeat(1, len(self.negatives))  # rays x N_phrase
    #     sims = torch.stack((repeated_pos, negative_vals), dim=-1)  # rays x N-phrase x 2
