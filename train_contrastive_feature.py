...
def training(dataset, opt, pipe, iteration, saving_iterations, checkpoint_iterations, debug_from):
    ...
    clip_gate = clip_gate.cuda()
    clip_gate.train()

    param_group = {'params': scale_gate.parameters(), 'lr': opt.feature_lr, 'name': 'f'}
    clip_param_group = {'params': clip_gate.parameters(), 'lr': opt.feature_lr, 'name': 'f'}

    feature_gaussians.optimizer.add_param_group(param_group)
    feature_gaussians.optimizer.add_param_group(clip_param_group)
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        with torch.no_grad():
            ...
            clip_features = viewpoint_cam.original_features.cuda().float()
            ...
            clip_features = clip_features[sort_indices, :]            
            clip_features = clip_features.unsqueeze(1).unsqueeze(2).repeat(1, sam_masks.shape[1], sam_masks.shape[2], 1).permute(0,3,1,2)
            ...
            sampled_ray = torch.logical_and(sampled_ray, ~non_mask_region) #[H, W] only sample mask_region
            ...
            per_pixel_mean_mask_size = per_pixel_mean_mask_size[sampled_ray] ##each sampled ray mask size [983]
            ...
            for idx, si in enumerate(sampled_scale_index):
                gt_clip_features.append(clip_features[si])
                gt_sam_masks.append(sam_masks_sampled_ray[si].bool())
            ...
            gt_sam_masks = torch.stack(gt_sam_masks, dim = 0)
            # N_scale S S
        ...
        clip_rendered_features = (clip_gate(rendered_features.permute(1,2,0)).permute(2,0,1)).unsqueeze(0).repeat([sampled_scales.shape[0],1,1,1])
        feature_with_scale = feature_with_scale * gates.unsqueeze(-1).unsqueeze(-1)
        sampled_clip_rendered_features = clip_rendered_features[:,:,sampled_ray]

        sampled_gt_clip_features = gt_clip_features[:,:,sampled_ray]
        ...
        sampled_clip_rendered_features = sampled_clip_rendered_features.permute([0,2,1])

        sampled_gt_clip_features = sampled_gt_clip_features.permute([0,2,1])

        sampled_clip_rendered_features_mask = sampled_clip_rendered_features[gt_sam_masks]

        sampled_gt_clip_features_mask = sampled_gt_clip_features[gt_sam_masks]


        sampled_clip_rendered_features_norm = sampled_clip_rendered_features_mask.norm(dim=-1, keepdim=True)
        normed_sampled_clip_rendered_features = sampled_clip_rendered_features_mask / (sampled_clip_rendered_features_norm + 1e-9)

        batch_gt_clip_features_norm = sampled_gt_clip_features_mask.norm(dim=-1, keepdim=True)
        normed_batch_gt_clip_features = sampled_gt_clip_features_mask / (batch_gt_clip_features_norm + 1e-9)

        clip_loss = torch.nn.functional.huber_loss(
            normed_sampled_clip_rendered_features, normed_batch_gt_clip_features, delta=1.25, reduction="none"
        )

        all_clip_loss = clip_loss.sum(dim=-1).mean()                
        ...
        loss = all_clip_loss + (- per_pixel_weight[:, sampled_mask_positive] * gt_corrs[:, sampled_mask_positive] * corr[:, sampled_mask_positive]).mean() \
        ...
        if iteration % 10 == 0:
            progress_bar.set_postfix({
                ...
                "CLIP_Loss": f"{all_clip_loss.item():.{3}f}",
    ...
    torch.save(clip_gate.state_dict(), os.path.join(scene.model_path, "point_cloud/iteration_{}/".format(iteration) + "clip_gate.pt"))
